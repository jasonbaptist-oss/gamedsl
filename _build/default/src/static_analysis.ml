...
let finalize r =
  { errors = List.rev r.errors; warnings = List.rev r.warnings }
...
let build_symtab prog r =
  ...
  List.iter (fun pl ->
    if Hashtbl.mem tbl.player_names pl.p_name then
      ...
      Hashtbl.add tbl.player_names pl.p_name pl
  ) prog.players;
  ...
  List.iter (fun m ->
    if Hashtbl.mem tbl.monster_names m.m_name then
      ...
      Hashtbl.add tbl.monster_names m.m_name m
  ) prog.monsters;
  ...
  List.iter (fun a ->
    if Hashtbl.mem tbl.ability_names a.a_name then
      ...
      Hashtbl.add tbl.ability_names a.a_name a
  ) prog.abilities;
  ...
  let collect_fn = function
    | SFnDecl f ->
      if Hashtbl.mem tbl.fn_names f.fn_name then
        add_error r (Printf.sprintf "Duplicate function name: \"%s\"" f.fn_name)
      else
        Hashtbl.add tbl.fn_names f.fn_name f
    | _ -> ()
  in
...
let check_global_structure prog r =
  if prog.players = [] then
    add_error r "At least one player block is required";
  ...
  if prog.prog_world.grid.width > 100 || prog.prog_world.grid.height > 100 then
    add_warning r "world.grid_size is unusually large (>100) — this may impact performance";
...
let collect_control_keys prog =
  List.concat_map (fun pl ->
    [ (pl.p_controls.up,    Printf.sprintf "player \"%s\" controls.up" pl.p_name);
      (pl.p_controls.down,  Printf.sprintf "player \"%s\" controls.down" pl.p_name);
      (pl.p_controls.left,  Printf.sprintf "player \"%s\" controls.left" pl.p_name);
      (pl.p_controls.right, Printf.sprintf "player \"%s\" controls.right" pl.p_name); ]
  ) prog.players

(* find the KEY field (if any) declared inside an ability's body *)
let ability_key ab =
  List.find_map (function
    | AbField (FKey k) -> Some k
    | _ -> None
  ) ab.a_body
...
let assigned_ability_names assign =
  List.filter_map (function
    | AbName n -> Some n
    | AbParam _ -> None   (* resolved dynamically, can't check key conflicts statically *)
...
let check_key_conflicts prog tbl r =
  ...
  List.iter (fun (k, where) ->
    match Hashtbl.find_opt seen k with
    | Some other_where ->
      add_error r (Printf.sprintf "Key conflict: \"%s\" used by both %s and %s" k other_where where)
  ...
  List.iter (fun assign ->
    let target_name =
      ...
      | TgString s -> Some s
      | TgIdent _ -> None  (* resolved at runtime inside a fn, skip static AUTO check *)
    in
          ...
          (match ability_key ab with
           ...
           | Some k ->
             if is_player tbl name && k = "AUTO" then
               add_error r (Printf.sprintf
                 "Ability \"%s\" assigned to player \"%s\" uses key AUTO, which is forbidden on player abilities"
                 aname name);
             if is_monster tbl name && k <> "AUTO" then
               add_error r (Printf.sprintf
                 "Ability \"%s\" assigned to monster \"%s\" uses key \"%s\", but monster abilities must use AUTO"
                 aname name k);
             if is_player tbl name then begin
               match Hashtbl.find_opt seen k with
               | Some other_where ->
                 add_error r (Printf.sprintf
                   "Key conflict: ability \"%s\" key \"%s\" collides with %s" aname k other_where)
               | None -> Hashtbl.add seen k (Printf.sprintf "ability \"%s\" (player \"%s\")" aname name)
             end)
      ) names
  ) prog.assigns
...
let scan_fields ab =
  let fp = {
    has_damage = false; has_range = false; has_shape = false;
    has_spread = false; has_key = false; has_activates_at = false;
    has_required_kills = false; has_damage_multiplier = false;
    has_damage_reduction = false; has_health_regen = false;
    has_speed_boost = false; shape_is_directional = false;
  } in
  let rec scan_stmts stmts =
    List.iter (fun s -> match s with
      ...
      | AbField (FShape sh)           -> fp.has_shape <- true;
...
let check_ability_types prog r =
  List.iter (fun ab ->
    ...
    (match ab.a_type with
     | Active ->
       if not fp.has_key    then add_error r (Printf.sprintf "Ability \"%s\" (active) is missing required field: key" name);
       if not fp.has_damage then add_error r (Printf.sprintf "Ability \"%s\" (active) is missing required field: damage" name);
       if not fp.has_range  then add_error r (Printf.sprintf "Ability \"%s\" (active) is missing required field: range" name);
       if not fp.has_shape  then add_error r (Printf.sprintf "Ability \"%s\" (active) is missing required field: shape" name);
       if fp.has_spread && not fp.shape_is_directional then
         add_error r (Printf.sprintf "Ability \"%s\" has spread but shape is not directional" name);
       if fp.has_activates_at then
         add_error r (Printf.sprintf "Ability \"%s\" (active) must not have activates_at — that is for type: timed" name);
       if fp.has_required_kills then
         add_error r (Printf.sprintf "Ability \"%s\" (active) must not have required_kills — that is for type: kill_unlocked" name)

     | Permanent ->
       if not (fp.has_damage_reduction || fp.has_health_regen
               || fp.has_speed_boost || fp.has_damage_multiplier) then
         add_error r (Printf.sprintf
           "Ability \"%s\" (permanent) needs at least one of: damage_reduction, health_regen, speed_boost, damage_multiplier" name);
       if fp.has_key    then add_error r (Printf.sprintf "Ability \"%s\" (permanent) must not have key" name);
       if fp.has_damage then add_error r (Printf.sprintf "Ability \"%s\" (permanent) must not have damage" name);
       if fp.has_range  then add_error r (Printf.sprintf "Ability \"%s\" (permanent) must not have range" name);
       if fp.has_shape  then add_error r (Printf.sprintf "Ability \"%s\" (permanent) must not have shape" name);
       if fp.has_spread then add_error r (Printf.sprintf "Ability \"%s\" (permanent) must not have spread" name);
       if fp.has_activates_at then add_error r (Printf.sprintf "Ability \"%s\" (permanent) must not have activates_at" name);
       if fp.has_required_kills then add_error r (Printf.sprintf "Ability \"%s\" (permanent) must not have required_kills" name)

     | Timed ->
       if not fp.has_key          then add_error r (Printf.sprintf "Ability \"%s\" (timed) is missing required field: key" name);
       if not fp.has_damage       then add_error r (Printf.sprintf "Ability \"%s\" (timed) is missing required field: damage" name);
       if not fp.has_range        then add_error r (Printf.sprintf "Ability \"%s\" (timed) is missing required field: range" name);
       if not fp.has_shape        then add_error r (Printf.sprintf "Ability \"%s\" (timed) is missing required field: shape" name);
       if not fp.has_activates_at then add_error r (Printf.sprintf "Ability \"%s\" (timed) is missing required field: activates_at" name);
       if fp.has_spread && not fp.shape_is_directional then
         add_error r (Printf.sprintf "Ability \"%s\" has spread but shape is not directional" name);
       if fp.has_required_kills then
         add_error r (Printf.sprintf "Ability \"%s\" (timed) must not have required_kills — that is for type: kill_unlocked" name)

     | KillUnlocked ->
       if not fp.has_key             then add_error r (Printf.sprintf "Ability \"%s\" (kill_unlocked) is missing required field: key" name);
       if not fp.has_required_kills  then add_error r (Printf.sprintf "Ability \"%s\" (kill_unlocked) is missing required field: required_kills" name);
       if not (fp.has_damage || fp.has_damage_multiplier) then
         add_error r (Printf.sprintf "Ability \"%s\" (kill_unlocked) needs at least one of: damage, damage_multiplier" name);
       if fp.has_activates_at then
         add_error r (Printf.sprintf "Ability \"%s\" (kill_unlocked) must not have activates_at — that is for type: timed" name))
  ) prog.abilities
    ...
    (match assign.asg_target with
     | TgString name ->
       if not (is_entity tbl name) then
         add_error r (Printf.sprintf "assign target \"%s\" is not a defined player or monster" name)
     | TgIdent _ -> ());  (* resolved dynamically inside a function — cannot check statically *)
    ...
    List.iter (fun ab ->
      ...
      | AbName aname ->
        if not (Hashtbl.mem tbl.ability_names aname) then
          add_error r (Printf.sprintf "assign references undefined ability \"%s\"" aname);
        if Hashtbl.mem seen aname then
          add_error r (Printf.sprintf "ability \"%s\" is assigned more than once in the same assign statement" aname)
...
let rec check_monsters prog tbl r =
  List.iter (fun m ->
    (match m.m_count with
     | EInt n when n < 0 ->
       add_error r (Printf.sprintf "monster \"%s\" has count %d — count must be >= 0" m.m_name n)
     | _ -> ());
    (match m.m_movement with
     | MvTowards pname ->
       if not (is_player tbl pname) then
         add_error r (Printf.sprintf "monster \"%s\" has movement: towards \"%s\", but \"%s\" is not a defined player"
  ...
  let rec check_actions actions =
    List.iter (fun a -> match a with
      | AMove (MTowards pname) ->
        if not (is_player tbl pname) then
          add_error r (Printf.sprintf "loop move: towards \"%s\" — \"%s\" is not a defined player" pname pname)
      | AIf (_, t, e) ->
        check_actions t;
  ...
  List.iter (fun s -> match s with
    | SSpawn sp ->
      if not (is_entity tbl sp.spawn_name) then
        add_error r (Printf.sprintf "spawn \"%s\" — not a defined player or monster" sp.spawn_name)
    | SIf (_, t, e) ->
...
let check_fn_decl tbl r f =
  ...
  List.iter (fun param ->
    if Hashtbl.mem seen param then
      add_error r (Printf.sprintf "function \"%s\" has a duplicate parameter name: \"%s\"" f.fn_name param)
  ...
  let rec walk_stmts stmts =
    List.iter (fun s -> match s with
      | SFnDecl inner ->
        add_error r (Printf.sprintf "function \"%s\" contains a nested function declaration \"%s\" — nested fn is not allowed"
                        f.fn_name inner.fn_name)
      | SFnCall call ->
        if call.call_name = f.fn_name then
          add_error r (Printf.sprintf "function \"%s\" calls itself — recursion is not allowed" f.fn_name);
        (match Hashtbl.find_opt tbl.fn_names call.call_name with
         | None -> add_error r (Printf.sprintf "call to undefined function \"%s\"" call.call_name)
         | Some target ->
           let expected = List.length target.fn_params in
           let got = List.length call.call_args in
           if expected <> got then
             add_error r (Printf.sprintf
               "function \"%s\" expects %d argument(s) but call provides %d" call.call_name expected got))
      | SIf (_, t, e) ->
        walk_stmts t;
...
let entity_ref_exists tbl = function
  | RefString name ->
    is_entity tbl name || Hashtbl.mem tbl.ability_names name
  | RefIdent _ -> true  (* assumed to be a valid parameter; full scope
...
let check_field_override tbl r entity ctx =
  ...
  | RefString name when not (entity_ref_exists tbl entity) ->
    add_error r (Printf.sprintf "field override on \"%s\" (%s) — not a defined player, monster, or ability" name ctx)
  | _ -> ()

let rec check_refs_in_stmts tbl r stmts =
  List.iter (fun s -> match s with
    | SFieldOverride (entity, _, _) -> check_field_override tbl r entity "stmt"
    | SIf (_, t, e) ->
...
let report prog =
  ...
  if result.errors = [] then
    print_endline "Static analysis: 0 errors."
  else begin
