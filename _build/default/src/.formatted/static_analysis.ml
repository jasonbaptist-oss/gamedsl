  ...
  let rec check_actions actions =
    List.iter
      (fun a ->
        ...
        | AMove _ | AWait _ | AAssign _ | AField _ | ASpawn _ | AFnCall _ -> ())
        (* <-- FIXED HERE *)
      actions
  in
...
and check_spawns_in_actions tbl r actions =
  List.iter
    (fun a ->
      ...
      | AMove _ | AWait _ | AAssign _ | AField _ | AFnCall _ -> ())
      (* <-- FIXED HERE *)
    actions
  ...
  and check_calls_in_actions actions =
    List.iter
      (fun a ->
        ...
        | AFnCall call -> (
            (* <-- FIXED HERE: Added proper validation for action-level fn calls! *)
            match Hashtbl.find_opt tbl.fn_names call.call_name with
            | None ->
                add_error r
                  (Printf.sprintf "call to undefined function \"%s\""
...
and check_refs_in_actions tbl r actions =
  List.iter
    (fun a ->
      ...
      | AMove _ | AWait _ | AAssign _ | ASpawn _ | AFnCall _ -> ())
      (* <-- FIXED HERE *)
    actions
