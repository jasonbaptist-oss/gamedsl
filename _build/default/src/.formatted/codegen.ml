...
and emit_action indent = function
  ...
  | AFnCall c -> emit_line ~indent (emit_fn_call_expr c)
(* <-- ADDED HERE *)
...
let emit_players players =
  ...
  List.iteri
    (fun i pl ->
      emit_line
        (Printf.sprintf "entity_registry[%s] = players[%d]" (py_str pl.p_name) i))
    players;
  emit_blank ()
...
let emit_ability ab =
  ...
  emit_line
    (Printf.sprintf
       ...
       img fn_name);
  emit_line
    (Printf.sprintf "entity_registry[%s] = ability_registry[%s]"
       (py_str ab.a_name) (py_str ab.a_name));
  emit_blank ()
...
let emit_monster m =
  ...
  | MvLoop loop ->
      ...
      emit_line
        (Printf.sprintf "entity_registry[%s] = monster_registry[%s]"
           (py_str m.m_name) (py_str m.m_name));
      emit_blank ()
  | simple ->
      let img = match m.m_img with Some s -> py_str s | None -> "None" in
      let pos =
...
let generate (prog : program) : string =
  ...
  if prog.post_stmts = [] then emit_line ~indent:1 "return"
  else List.iter (emit_stmt 1) prog.post_stmts;
  emit_blank ();

  emit_line "if __name__ == \"__main__\":";
  emit_line ~indent:1
    "run_game(players, monster_registry, obstacles, check_win_condition,      \
     GRID_WIDTH, GRID_HEIGHT, DURATION, game_state=game_state, \
     script=_game_script)";
  Buffer.contents buf
