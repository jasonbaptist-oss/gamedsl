...
let test_lex_number_int _ = assert_equal (INT_LIT 42) (first "42")
let test_lex_number_zero _ = assert_equal (INT_LIT 0) (first "0")
let test_lex_number_large _ = assert_equal (INT_LIT 9999) (first "9999")
let test_lex_number_float _ = assert_equal (FLOAT_LIT 3.14) (first "3.14")
...
let test_lex_number_duration _ = assert_equal (DURATION_LIT 120) (first "120s")
let test_lex_number_duration_one _ = assert_equal (DURATION_LIT 1) (first "1s")
let test_lex_number_percent _ = assert_equal (PERCENT_LIT 30) (first "30%")
...
let test_lex_string_empty _ = assert_equal (STRING_LIT "") (first "\"\"")

let test_lex_filepath_slash _ =
  assert_equal (FILEPATH_LIT "assets/goblin.png")
    (first "\"assets/goblin.png\"")
...
let test_lex_string_newline_inside _ =
  assert_raises
    (LexError
       ("Unterminated string- newline inside quotes ", { line = 1; col = 1 }))
    (fun () -> toks "\"bad\nnewline\"")
...
let test_lookup_keyword_world _ = assert_equal WORLD (first "world")
let test_lookup_keyword_player _ = assert_equal PLAYER (first "player")
let test_lookup_keyword_active _ = assert_equal ACTIVE (first "active")
let test_lookup_keyword_true _ = assert_equal TRUE (first "true")
let test_lookup_keyword_false _ = assert_equal FALSE (first "false")
let test_lookup_keyword_spawn _ = assert_equal SPAWN (first "spawn")
let test_lookup_keyword_infinite _ = assert_equal INFINITE (first "infinite")
...
let test_is_key_space _ = assert_equal (KEY_LIT "SPACE") (first "SPACE")
let test_is_key_auto _ = assert_equal (KEY_LIT "AUTO") (first "AUTO")
let test_is_key_w _ = assert_equal (KEY_LIT "W") (first "W")
...
let test_op_lbrace _ = assert_equal LBRACE (first "{")
let test_op_rbrace _ = assert_equal RBRACE (first "}")
let test_op_eq _ = assert_equal EQ (first "==")
let test_op_assign _ = assert_equal ASSIGN_OP (first "=")
let test_op_neq _ = assert_equal NEQ (first "!=")
let test_op_geq _ = assert_equal GEQ (first ">=")
let test_op_leq _ = assert_equal LEQ (first "<=")
let test_op_gt _ = assert_equal GT (first ">")
let test_op_lt _ = assert_equal LT (first "<")
...
let test_tokenize_empty _ = assert_equal [ EOF ] (toks "")
...
let test_to_string_world _ = assert_equal "WORLD" (token_to_string WORLD)
...
let test_to_string_true _ = assert_equal "TRUE" (token_to_string TRUE)
let test_to_string_false _ = assert_equal "FALSE" (token_to_string FALSE)
let test_to_string_spawn _ = assert_equal "SPAWN" (token_to_string SPAWN)
...
let skip_tests =
  ...
  >::: [
         ...
         "skip unterminated block comment"
         >:: test_skip_unterminated_block_comment;
       ]
