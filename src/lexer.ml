...
let peek s = if is_eof s then '\x00' else s.src.[s.pos]

let peek2 s =
  if s.pos + 1 >= String.length s.src then '\x00' else s.src.[s.pos + 1]

let advance s =
  if not (is_eof s) then (
    if s.src.[s.pos] = '\n' then (
      s.line <- s.line + 1;
      s.col <- 1)
    else s.col <- s.col + 1;
...
let lookup word =
  match List.assoc_opt word keywords with Some b -> b | None -> IDENT word
...
let is_filepath_char c = is_alnum c || c = '/' || c = '\\' || c = '.' || c = '-'
...
let lex_number s pos =
  ...
  if (not (is_eof s)) && peek s = '.' && is_digit (peek2 s) then (
    ...
    if Buffer.length buf2 = 0 then
      ...
      let height = int_of_string (Buffer.contents buf2) in
      GRID_DIM_LIT (int_part, height))
  else if
    (not (is_eof s))
    && peek s = 's'
    && (is_eof { s with pos = s.pos + 1 } || not (is_alnum (peek2 s)))
  then (
    advance s;
...
let lex_ident s =
  ...
  let rec finish_word s =
    if (not (is_eof s)) && is_alnum (peek s) then (
      Buffer.add_char buf (next s);
      finish_word s)
  in
  finish_word s;
...
type located_token = { token : token; pos : position }
...
let tokenize src =
  ...
  let rec loop acc =
    ...
    if tok = EOF then List.rev new_acc else loop new_acc
  in
  loop []
...
let print_tokens tokens =
  List.iter
    (fun { token; pos } ->
      Printf.printf "  [%3d:%2d]  %s\n" pos.line pos.col (token_to_string token))
    tokens
